...
SHA256_Hasher::SHA256_Hasher() { reset(); }
...
void SHA256_Hasher::update(std::span<const uint8_t> data) {
...
std::vector<uint8_t> SHA256_Hasher::finish() {
  uint64_t totalBits =
...
__attribute__((always_inline)) void SHA256_Hasher::process_block(
    const uint8_t* data) {
  uint32_t w[64];
